...
def split_data(df):
    ...
    train, test1, test2 = np.array(
        np.ceil([0.7 * num_of_ill_patients,
                 0.8 * num_of_ill_patients,
                 0.9 * num_of_ill_patients])).astype(int)

    train_ill, test1_ill, test2_ill, test3_ill = np.split(ill, [train, test1, test2])
    ...
    train_healthy, test1_healthy, test2_healthy, test3_healthy = np.split(healthy, [train, test1, test2])
    train = df[
